using Api.Data;
using Microsoft.EntityFrameworkCore;
using Api.Models;

namespace Api.Services.GenericService
{
  public class GenericService<TEntity> : IGenericService<TEntity> where TEntity : ModelBase
  {
    protected TyTContext context;
    internal DbSet<TEntity> entitySet;

    public GenericService(TyTContext context)
    {
        this.context = context;
        entitySet = context.Set<TEntity>();
    }

    public virtual TEntity Add(TEntity entity) => entitySet.Add(entity).Entity;

    public void AddMany(TEntity[] entities) => entitySet.AddRange(entities);

    public virtual async Task<int> Delete(int id) => await entitySet.Where(e => e.Id == id).ExecuteDeleteAsync();

    public virtual async Task<List<TEntity>> GetAllAsync() => await entitySet.ToListAsync();

    public virtual async Task<TEntity?> GetAsync(int id) => await entitySet.AsNoTracking().Where(e => e.Id == id).FirstOrDefaultAsync();

    public virtual TEntity Update(TEntity entity) => entitySet.Update(entity).Entity;
  }
}